...
class CannonSimulator:
    ...
    def build_controls(self, parent):
        ...
        tk.Label(form_frame, text="Sākuma ātrums v₀ (m/s):", font=("Arial", 11),
                 bg="#dce7f2").grid(row=0, column=0, sticky="w", pady=6)
        self.entry_v0 = ttk.Entry(form_frame, width=18)
        self.entry_v0.grid(row=0, column=1, pady=6)
        self.entry_v0.insert(0, "20")

        tk.Label(form_frame, text="Leņķis α (°):", font=("Arial", 11),
                 bg="#dce7f2").grid(row=1, column=0, sticky="w", pady=6)
        self.entry_angle = ttk.Entry(form_frame, width=18)
        self.entry_angle.grid(row=1, column=1, pady=6)
        self.entry_angle.insert(0, "45")

        tk.Label(form_frame, text="Masa m (kg):", font=("Arial", 11),
                 bg="#dce7f2").grid(row=2, column=0, sticky="w", pady=6)
        ...
        gravity_check = ttk.Checkbutton(
            form_frame,
            text="Ieslēgt gravitāciju",
            variable=self.gravity_var
        )
        gravity_check.grid(row=3, column=0, columnspan=2, sticky="w", pady=10)
    ...
    def draw_static_scene(self):
        ...
        self.canvas.create_rectangle(0, self.ground_y, self.canvas_width, self.canvas_height,
                                     fill="#cfe8b4", outline="")
        self.canvas.create_line(0, self.ground_y, self.canvas_width, self.ground_y, width=3, fill="#3d5a2b")


        self.canvas.create_text(35, 25, text="y", font=("Arial", 12, "bold"))
        self.canvas.create_text(self.canvas_width - 15, self.ground_y + 20, text="x", font=("Arial", 12, "bold"))

        self.canvas.create_line(self.origin_x, self.ground_y, self.origin_x, 20, width=2, arrow="last")
        self.canvas.create_line(self.origin_x, self.ground_y, self.canvas_width - 20, self.ground_y, width=2, arrow="last")
    ...
    def draw_cannon(self, angle_deg=45):
        ...
        self.canvas.create_oval(base_x - 20, base_y - 20, base_x + 20, base_y + 20,
        ...
        self.canvas.create_line(base_x, base_y, end_x, end_y, width=12, fill="#666666", capstyle="round")
    ...
    def calculate_trajectory(self, v0, angle_deg, gravity_on):
        angle_rad = math.radians(angle_deg)
    ...
    def display_results(self, v0, angle_deg, mass, v0x, v0y, gravity_on):
        angle_rad = math.radians(angle_deg)
    ...
    def start_simulation(self):
        ...
        gravity_on = self.gravity_var.get()

        self.clear_canvas(redraw_only=True)
        self.draw_cannon(angle_deg)

        points, v0x, v0y = self.calculate_trajectory(v0, angle_deg, gravity_on)
        self.display_results(v0, angle_deg, mass, v0x, v0y, gravity_on)

        scale = self.get_scale(points)

        self.animation_points = [self.physics_to_screen(x, y, scale) for x, y in points]
        self.current_index = 0
        self.path_segments = []
        ...
        if self.animation_points:
            ...
            self.projectile = self.canvas.create_oval(x - 7, y - 7, x + 7, y + 7,
    ...
    def animate(self):
        ...
        if self.current_index > 0:
            ...
            segment = self.canvas.create_line(prev_x, prev_y, x, y, fill="#ff7b00", width=2)
